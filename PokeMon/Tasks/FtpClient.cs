...
namespace FTP
{
    ...
    public class FTPclient
    {
        ...
        private FtpWebRequest GetRequest(string URI)
        {
            ...
            // Doesn't appear to work on the WebHost4Life FTP server
    ...
    /// 
    [Serializable]
	public class FTPfileInfo : IComparable
	{
		...
		public FTPfileInfo(string line, string path)
		{
			...
			else
			{
				...
				try
				{
                    ...
                    // this is the original code, I found it doesn't work for the date values returned by our
                    // ftp server
					//_fileDateTime = DateTime.Parse(m.Groups["timestamp"].Value);

                    // Converts date formats like "Dec 12 09:34" in 24 hour time and "Dec 12 2007".  
                    // Anything it can't parse it will fill in with today's value 
                    _fileDateTime = DateTime.ParseExact(m.Groups["timestamp"].Value.Replace("  ", " "), new string[] {"MMM d HH:mm", "MMM d yyyy"}, formatInfo, DateTimeStyles.None);
				}
				catch (Exception)
				{
        ...
        //public FTPfileInfo(SerializationInfo info, StreamingContext ctxt)
        //{
        //    _path = (string)info.GetValue("Path", typeof(string));
        //    _filename = (string)info.GetValue("FullName", typeof(string));
        //    _permission = (string)info.GetValue("Permission", typeof(string));
        //    _fileDateTime = (DateTime)info.GetValue("Time", typeof(DateTime));
        //    _size = (long)info.GetValue("Size", typeof(long));
        //}
        ...
        //    info.AddValue("Permission", Permission);
        //    info.AddValue("Time", this.FileDateTime);
        ...
        {
            FTPfileInfo rhs = (FTPfileInfo)obj;
        ...
        public override string ToString()
        {
            return "Full Name: " + this.FullName + "\n" +
                "Last Modified: " + this.FileDateTime + "\n" +
                "Size: " + this.Size + "\n" +
        ...
        public int CompareTo(object obj)
        {
            ...
            if (this.FullName == rhs.FullName &&
                this.Permission == rhs.Permission &&
                this.FileDateTime == rhs.FileDateTime &&
                this.Size == rhs.Size)
            {
                return 0;
            }
            else if (this.FileDateTime < rhs.FileDateTime)
            {
                return -1;
            }
            else
	...
	public class FTPdirectory : List<FTPfileInfo>
	{
		...
		public FTPdirectory(string dir, string path)
		{
			...
			{
				//parse if it's not a blank line and it's not the total summary
